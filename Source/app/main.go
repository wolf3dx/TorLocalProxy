...
func собратьЭкран(окно fyne.Window, служба *service.Service) *экран {
	...
	э.мосты.SetMinRowsVisible(4)
	// Перенос по строке обязателен, а не для красоты: строки мостов
	// длинные, и без переноса поле растягивается по ширине сильнее
	// экрана телефона. Вместе с ним уезжают вправо и подписи кнопок —
	// поймано на эмуляторе, где «Встроенные» и «Убрать тип» оказались
	// обрезаны.
	э.мосты.Wrapping = fyne.TextWrapBreak
	сохранить := widget.NewButtonWithIcon("Сохранить", theme.DocumentSaveIcon(), func() {
	...
	// Встроенные мосты — выход из положения, когда своих мостов нет или
	// они перестали работать. Заказывать их не надо: у snowflake и
	// meek_lite строка одна на всех, и точно эти строки лежат внутри
	// каждого Tor Browser.
	//
	// Кнопка нужна именно потому, что obfs4 на мобильных сетях узнают и
	// глушат: там, где он мёртв, человеку нечего вписать в поле — а
	// теперь есть что нажать.
	встроенные := widget.NewButtonWithIcon("Встроенные",
		theme.DownloadIcon(), func() {
			э.мосты.SetText(bridges.Builtin())
			if err := служба.SaveBridges(э.мосты.Text); err != nil {
				сообщить(окно, "Не сохранилось: "+err.Error())
				return
			}
			э.обновитьПодписьМостов()
			// Снимаем фокус с поля: иначе на телефоне вылезает экранная
			// клавиатура и закрывает собой кнопку «Подключить» — ровно
			// ту, которую человеку нужно нажать следующей. Поймано при
			// проверке на эмуляторе: я сам не смог до неё дотянуться.
			окно.Canvas().Unfocus()
			сообщить(окно, "Вписаны встроенные мосты. Нажмите «Подключить».")
		})

	// Мосты приходится держать наборами: на одной сети работает один
	// тип, на другой — другой. Отсюда и кнопки: набор берут из файла,
	// уносят в файл и выбрасывают по одному типу.
	сохранитьМосты := func(текст string) error { return служба.SaveBridges(текст) }
	// Файлы и чистка спрятаны за одну кнопку, а не разложены в ряд.
	// Причина не в красоте: три кнопки с иконками не влезают в ширину
	// телефона, и подписи обрезаются по правому краю — это видно на
	// эмуляторе. В отдельном окне им хватает места на полное имя.
	ещё := widget.NewButtonWithIcon("Файлы и очистка", theme.MenuIcon(), func() {
		э.показатьДействия(окно, сохранитьМосты)
	})

	э.плашкаМостов = widget.NewCard("Мосты", "", container.NewVBox(
		э.мосты,
		container.NewGridWithColumns(2, сохранить, встроенные),
		ещё,
	))
	э.обновитьПодписьМостов()
...
func (э *экран) обновитьПодписьМостов() {
	...
	default:
		// Типы называются не для красоты: когда подключение не идёт,
		// первый вопрос — чем пробуем. Порядок в подписи тот же, в
		// котором приложение будет перебирать типы, так что подпись
		// заодно показывает, что пойдёт первым.
		э.плашкаМостов.SetSubTitle(strings.Join(bridges.Counts(годные), " · "))
	}
}
