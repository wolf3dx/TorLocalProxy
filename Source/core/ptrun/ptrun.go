...
// процесс. Делается один раз и не отменяется: библиотеки пишут и после
// остановки транспорта, и терять эти строки тоже не хочется.
var перехватЖурнала sync.Once
...
func (в вЖурнал) Write(данные []byte) (int, error) {
...
func (л *Local) Start(имена []string, stateDir string) ([]Plugin, error) {
	...
	if л.настройки.Log != nil {
		перехватЖурнала.Do(func() {
			log.SetOutput(вЖурнал{писать: л.настройки.Log})
			log.SetFlags(0)
		})
	}
